<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
  ...
  <!-- created by Yves Lorphelin, largely inspired by the nunitsumary.xsl (see nantcontrib.sourceforge.net)-->
    <xsl:variable name="sourceurlprefix" select="//coverage/@sourceurlprefix"/>	
	...
	<xsl:template name="seqpnt">
		...
		<div>
		  ...
		  <table cellpadding="3" cellspacing="0" border='1' width="100%" bordercolor="black">
			<tr>
					...
					<td class="datacell" style="text-align:left">
						<a target="_new">
              				<xsl:attribute name = "href">
								<xsl:value-of select = "$sourceurlprefix"/>
								<xsl:value-of select = "@document"/>#L<xsl:value-of select="@line" />
							</xsl:attribute>
	...
	<xsl:template name="ClassSummary">
		...
		<a><xsl:attribute name="name"><xsl:value-of select="$class"/></xsl:attribute></a>
		<table width='90%'>
			<tr>
