...
namespace Spring.Extension.Tests
{
    public static class TestHelper
    {
        ...
        private static object RunMethod(Type t, string strMethod, Type[] types,
                                        object objInstance, object[] aobjParams, BindingFlags eFlags)
        {
            MethodInfo m = t.GetMethod(strMethod, eFlags, null, types, null);
            if (m == null)
            {
                StringBuilder sb = new StringBuilder("There is no method '");
                sb.Append(strMethod).Append('(');
                for (int i = 0; i < types.Length; i++ )
                {
                    if (i > 0) sb.Append(',');
                    sb.Append(types[i].FullName);
                }
                sb.Append(")' for type '").Append(t).Append("'.");
        ...
        public static object RunInstanceMethod(
            object instance, string methodName, Type[] types, params object[] parameters)
        {
            return RunInstanceMethod(instance.GetType(), methodName, types, instance, parameters);
        }

        public static object RunInstanceMethod(
            Type t, string strMethod, Type[] types, object objInstance, params object[] aobjParams)
        {
            BindingFlags eFlags =
                BindingFlags.Instance |
                BindingFlags.Public |
            ...
            try
            {
                return RunMethod(t, strMethod, types, objInstance, aobjParams, eFlags);
            }
            catch (TargetInvocationException ex)
            {
                Exception inner = ex.InnerException;
            ...
            Type t, string strMethod, Type[] types, object objInstance, params object[] aobjParams)
        {
            ...
            try
            {
                return RunMethod(t, strMethod, types, null, aobjParams, eFlags);
            }
            catch (TargetInvocationException ex)
            {
                Exception inner = ex.InnerException;
